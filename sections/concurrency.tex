...
thread, it is backed by a corresponding kernel thread, which has its own kernel stack \parencite{mckusick2014}. The
kernel creates the illusion of concurrency by scheduling system resources among the set of ready processes
\parencite{mckusick2014}. Multiple threads of the same or different processes can be executed in true parallel fashion
on a multiprocessor, which has different CPUs \parencite{mckusick2014}. Those processors share a single instance of
...
\parencite{mckusick2014}. NetBSD instead treats processes and threads alike as LWPs \parencite{rasiukevicius2009}. That
...
\parencite{netbsd_pserialzie}. Table \ref{netbsd-context-matrix} lists which primitives are valid in each execution
...
\parencite{mckusick2014_sync}. It prevents context switches on the current CPU, so neither preemption nor migration can occur
while inside it \parencite{mckusick2014_sync}. Critical sections carry less overhead than locks and allow for short operations on
per-CPU run queues and allocators \parencite{mckusick2014_sync}. However, they protect only the current CPU and does not provide
for system-wide shared data \parencite{mckusick2014_sync}. This is significant because FreeBSD's preemptive kernel allows a thread
...
compare-and-swap to acquire or release \parencite{mckusick2014_sync}. A contested release is far more expensive because the
kernel must locate the turnstile, lock the turnstile chain, and wake the waiters \parencite{mckusick2014_sync}.
Adaptive spinning mitigates this by letting a waiter spin when the holder is running on another CPU instead of
entering the costly block-and-wakeup cycle \parencite{mckusick2014_sync}. Locking too finely appeared during the netperf work
...
\ref{resource-scalability}, which reached scalability beyond 100 cores for many workloads \parencite{freebsd_vs_dragonfly}.

...
\parencite{mckusick2014_process}. System resources such as pipes, sockets, files, and disk devices can be accessed by the
\textit{descriptor array}, which is one of the kernel resources FreeBSD manages aside from credentials and signal states
\parencite{mckusick2014_process}. Each process is assigned to its own descriptor array listing down the devices it can
use \parencite{mckusick2014_process}. Each thread is assigned to its own set of registers, program counter, and stack \parencite{mckusick2014_process}.
...
of disproportionally allotting CPU time to the highest priority process \parencite{roberson_ule}.
ULE achieves this with per-CPU run queues and CPU topology model \parencite{roberson_ule}.
...
When a process is terminated, the \texttt{exit()} routine is called to kill all threads associated with that process \parencite{mckusick2014_process}.
...
to the zombie list specified by \texttt{zombproc} \parencite{mckusick2014_process}. The accumulated resource usage is bundled into a
\texttt{p\_ru} structure which is retained for the parent to be retrieved by calling the \texttt{wait4()} function \parencite{mckusick2014_process}.
...
\parencite{roberson_ule}. This is evaluated on three events: a thread becomes runnable, a CPU goes idle, and the periodic rebalancer
...
the same physical execution units \parencite{roberson_ule}. When compared against Linux's CFS, it was found by \textcite{bouron2018}
...
attempt to reacquire the same mutex being held by the interrupted thread \parencites{mckusick2014_process}{freebsd_mutex}. This, however,
comes at the cost of increased interrupt latency \parencites{mckusick2014_process}{freebsd_mutex}. FreeBSD also incorporates an
...
the lock owner and waiting threads in a \textbf{tunstile} data structure \parencite{mckusick2014_process}. The waiting threads
...
recursively propagated to the owner if a waiter has a higher priority than the current lock holder \parencite{mckusick2014_process}.
...
\parencite{mckusick2014_process}. This ensures that the number of simultaneously active turnstiles never exceeds the number of
threads \parencite{mckusick2014_process}. This also controls potential memory overhead fluctuations \parencite{mckusick2014_process}.
...
blocking forks, exits, and context switches at the same time in the process \parencite{mckusick2014_process}. For lock-level
...
\parencite{roberson_ule}. This balance is also achieved by assigning smaller time slices to high-priority and interactive
...
result in potentially spending a lot of CPU time spinning under high contention before going back to sleeping \parencite{netbsd_locking}.
...
is replaced by a 1:1 threading model \parencite{williams_scheduler}. As discussed in Section \ref{load-balancing-distribution}, the

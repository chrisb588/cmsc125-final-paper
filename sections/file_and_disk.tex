\subsection{File System Structure and Organization} \label{file-structure-and-hierarchy}
...
\begin{itemize}
	...
	\item \textbf{Superblock}: The superblock stores information about the filesystem, which can include the size and
	      ...
	      drives for instances, the offsets are configured such that a replica of the superblock is present in each platter.
...
It is evident that the approaches of FreeBSD's ZFS and NetBSD's FFS in file management is largely different. NetBSD's FFS
...
\subsection{File Access Methods and Operations} \label{file-access-methods} % comparison
...
It can be seen from the two systems

\subsection{Directory Structures} \label{dir-struct} % comparison
...
serve as common mount points of the filessytem \parencite{freebsd_dir_struct}. The children filesystems and the mount points they are
mounted are recorded in the \texttt{/etc/fstab} directory \parencite{freebsd_dir_struct}. The \texttt{namei()} function handles
...
FreeBSD perform file lookups with the \texttt{VOP\_LOOKUP()} function \parencite{freebsd_voplookup}. It specifically searches for a
...
\begin{table}[!htb]
	...
	\begin{tabular}{|c|p{0.6\textwidth}|}
		...
		\texttt{/dev}         & Contains device nodes, which are entrypoints to devices that are connected to the system \parencite{freebsd_intro}     \\ \hline
...
\subsection{Disk Space Allocation and Free Space Management} \label{space-allocation-and-free-space} % comparison
...
\subsection{File Protection and Security} \label{file-protection-security} % comparison
...
a total of 57 separate roles \parencite{crooks2009}.

\subsection{Reliability and Crash Recovery} \label{reliability-and-crash-recovery} % comparison
...
\subsection{Disk Scheduling and I/O Management} \label{disk-scheduling-io} % comparison

Earlier releases of FreeBSD uses primitive disk schedulers such as elevator and C-LOOK schedulers \parencite{rizzo2009}.
...
\subsection{Performance Considerations} \label{performance-considerations}

FreeBSD's ZFS support three caching strategies: ARC, L2ARC, and SLOG \parencite{khurshid2025}. The
